...
def read_dataset_wsd(path: str) -> List[str]:

    labels = []

    with open(path) as f:
        for obj in f:
            labels.append(obj.split(' ')[1].strip())
    return labels

def main(test_path: str, test_path_wsd: str, endpoint: str, batch_size=32):
    ...
    if test_path_wsd:
        try:
            wsd_labels = read_dataset_wsd(test_path_wsd)
        except FileNotFoundError as e:
            logging.error(f'Evaluation crashed because {test_path_wsd} does not exist')
            exit(1)
        except Exception as e:
            logging.error(f'Evaluation crashed. Most likely, the file you gave is not in the correct format')
            logging.error(f'Printing error found')
    ...
    predictions_wic, predictions_wsd = [], []
    ...
    for i in range(0, len(sentence_pairs), batch_size):
        ...
        try:
            ...
            predictions_wic += response['predictions_wic']
            predictions_wsd.extend([sent for pred in response['predictions_wsd'] for sent in pred])
        except KeyError as e:
            logging.error(f'Server response in wrong format')
            logging.error(f'Response was: {response}')
            logging.error(e, exc_info=True)
    ...
    pred_distribution = count(predictions_wic)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-file_wsd", type=str, help='File containing data you want to evaluate upon for wsd', default=None)
    args = parser.parse_args()

    main(
        test_path=args.file,
        test_path_wsd=args.file_wsd,
